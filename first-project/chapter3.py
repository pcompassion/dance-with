...
class Introduction(TeacherStudentsScene):
    def construct(self):
        ...
        self.play(
            ...
            ApplyFunction(lambda m: m.center().to_edge(UP), linear_transformations),
        )


class MatrixVectorMechanicalMultiplication(NumericalMatrixMultiplication):
    CONFIG = {"left_matrix": [[1, -3], [2, 4]], "right_matrix": [[5], [7]]}
...
class DescribeTransformation(Scene):
    ...
    def add_title(self):
        ...
        self.wait()
        self.play(GrowFromCenter(brace), Write(function), ApplyMethod(linear.fade))
    ...
    def show_function(self):
        ...
        num_outputs = VMobject(*[OldTex(str(num**2)) for num in nums])
        ...
        for mob in f_point, num_outputs:
            self.play(Transform(num_inputs, mob, lag_ratio=0.5))
        self.wait()

        self.play(FadeOut(num_inputs), Transform(f_of_x, L_of_v))
        self.play(Write(input_vect), Write(vector_input_words))
        self.wait()
        for mob in f_point, output_vect:
            self.play(Transform(input_vect, mob, lag_ratio=0.5))
...
class WhyConfuseWithTerminology(TeacherStudentsScene):
    def construct(self):
        ...
        self.play(
            *[ApplyMethod(student.change_mode, "happy") for student in other_students]
        )
        self.random_blink()
        self.wait()
...
class ThinkinfOfFunctionsAsGraphs(VectorScene):
    def construct(self):
        ...
        self.play(ShowCreation(point), Write(point_label), run_time=1)
        self.wait()
...
class TransformJustOneVector(VectorScene):
    def construct(self):
        ...
        t_matrix = [[0, -1], [2, -1]]
        v1 = Vector(v1_coords)
        v2 = Vector(np.dot(np.array(t_matrix).transpose(), v1_coords), color=PINK)
        ...
        self.remove(v2)
        self.play(
            Transform(v1.copy(), v2, path_arc=-np.pi / 2, run_time=3),
            ApplyMethod(v1.fade),
        )
        self.wait()
...
class TransformManyVectors(LinearTransformationScene):
    ...
    def construct(self):
        ...
        if self.use_dots:
            self.play(
                Transform(
                    vectors, self.vectors_to_dots(vectors), run_time=3, lag_ratio=0.5
                )
            )
            transformed_vectors = self.vectors_to_dots(transformed_vectors)
            self.wait()
        self.play(
            Transform(vectors, transformed_vectors, run_time=3, path_arc=-np.pi / 2)
        )
        self.wait()
        if self.use_dots:
            self.play(
                Transform(
                    vectors, self.dots_to_vectors(vectors), run_time=2, lag_ratio=0.5
                )
            )
            self.wait()
    ...
    def vectors_to_dots(self, vectors):
        return VMobject(
            *[Dot(v.get_end(), color=v.get_color()) for v in vectors.split()]
        )

    def dots_to_vectors(self, dots):
        return VMobject(
            *[Vector(dot.get_center(), color=dot.get_color()) for dot in dots.split()]
        )
...
class TransformInfiniteGrid(LinearTransformationScene):
    ...
    def construct(self):
        self.setup()
        self.play(ShowCreation(self.plane, run_time=3, lag_ratio=0.5))
        self.wait()
...
class CrazyTransformation(ApplyComplexFunction):
    CONFIG = {"function": lambda z: np.sin(z) ** 2 + np.sinh(z)}
...
class LookToWordLinear(Scene):
    def construct(self):
        ...
        self.add(faded_linear, transformation, transformation_brace, function)
        self.wait()
        self.play(
...
class IntroduceLinearTransformations(LinearTransformationScene):
    ...
    def construct(self):
        ...
        self.wait()
        self.play(
            Write(origin_rule, run_time=2), ShowCreation(arrow), GrowFromCenter(dot)
        )
        self.wait()
...
class SimpleLinearTransformationScene(LinearTransformationScene):
    CONFIG = {"show_basis_vectors": False, "transposed_matrix": [[2, 1], [1, 2]]}
...
class SimpleNonlinearTransformationScene(LinearTransformationScene):
    CONFIG = {"show_basis_vectors": False, "words": "Not linear: some lines get curved"}
...
class MovingOrigin(SimpleNonlinearTransformationScene):
    CONFIG = {"words": "Not linear: Origin moves"}
...
class SneakyNonlinearTransformationExplained(SneakyNonlinearTransformation):
    CONFIG = {"words": "Not linear: diagonal lines get curved"}
...
class GridLinesRemainParallel(SimpleLinearTransformationScene):
    CONFIG = {"transposed_matrix": [[3, 0], [1, 2]]}
...
class FollowIHatJHat(LinearTransformationScene):
    CONFIG = {"show_basis_vectors": False}

    def construct(self):
        self.setup()
        i_hat = self.add_vector([1, 0], color=X_COLOR)
        i_label = self.label_vector(
            i_hat, "\\hat{\\imath}", color=X_COLOR, label_scale_factor=1
        )
        j_hat = self.add_vector([0, 1], color=Y_COLOR)
        j_label = self.label_vector(
            j_hat, "\\hat{\\jmath}", color=Y_COLOR, label_scale_factor=1
        )
...
class TrackBasisVectorsExample(LinearTransformationScene):
    ...
    def introduce_vector(self):
        ...
        pre_def = VMobject(
            ...
            VMobject(*[mob.copy() for mob in coords.get_mob_matrix().flatten()]),
        )
        self.play(Transform(pre_def, v_def, run_time=2, lag_ratio=0))
        self.remove(pre_def)
    ...
    def show_linear_combination(self, clean_up=True):
        ...
        self.play(
            ApplyFunction(lambda m: m.scale(self.v_coords[0]).fade(0.3), i_hat_copy)
        )
        self.play(
            ApplyFunction(lambda m: m.scale(self.v_coords[1]).fade(0.3), j_hat_copy)
        )
...
class WatchManyVectorsMove(TransformManyVectors):
    def construct(self):
        ...
        self.play(Transform(dots, vectors, lag_ratio=0.5, run_time=2))
        self.remove(dots)
        for v in vectors.split():
...
class DeduceResultWithGeneralCoordinates(Scene):
    def construct(self):
        ...
        row1 = OldTex("1x + 3y")
        row2 = OldTex("-2x + 0y")
        VMobject(row1.split()[0], row2.split()[0], row2.split()[1]).set_color(X_COLOR)
        VMobject(
            row1.split()[1], row1.split()[4], row2.split()[2], row2.split()[5]
        ).set_color(YELLOW)
        VMobject(row1.split()[3], row2.split()[4]).set_color(Y_COLOR)
...
class MatrixVectorMultiplication(LinearTransformationScene):
    ...
    def build_to_matrix(self):
        ...
        abstract_matrix = np.append(
            i_coords.get_mob_matrix(), j_coords.get_mob_matrix(), axis=1
        )
        concrete_matrix = Matrix(
            copy.deepcopy(abstract_matrix), add_background_rectangles_to_entries=True
        )
        ...
        everything = VMobject(*self.get_mobjects())
        self.play(FadeOut(everything), Animation(concrete_matrix))
    ...
    def label_matrix(self, matrix):
        ...
        self.play(Write(j_message, run_time=1.5), ShowCreation(j_arrow))
        self.wait()
        self.play(
            *list(
                map(
    ...
    def multiply_by_vector(self, matrix):
        ...
        self.play(Write(vector), GrowFromCenter(brace), Write(words), run_time=1)
        self.wait()
        ...
        formula = VMobject(v1.copy(), col1, OldTex("+"), v2.copy(), col2)
        ...
        self.play(
            FadeOut(brace),
            FadeOut(words),
            Transform(formula_start, formula, run_time=2, lag_ratio=0),
        )
        self.wait()
    ...
    def show_result(self, formula):
        ...
        self.play(Write(equals, run_time=1), Write(final_sum))
        self.wait()
    ...
    def reposition_matrix_and_vector(self, matrix, vector, formula):
        ...
        self.play(
            Transform(start_state, end_state, lag_ratio=0), Write(equals, run_time=1)
        )
        self.wait()
        self.play(FadeIn(brace), FadeIn(brace_words), lag_ratio=0.5)
        self.wait()
...
class ColumnsToBasisVectors(LinearTransformationScene):
    ...
    def move_matrix_columns(self, transposed_matrix, vector_coords=None):
        ...
        i_coords_start = VMobject(
            matrix_background.copy(), col1.copy(), matrix_brackets.copy()
        )
        ...
        j_coords_start = VMobject(
            matrix_background.copy(), col2.copy(), matrix_brackets.copy()
        )
        ...
        transform_matrix2 = np.dot(matrix, np.linalg.inv(transform_matrix1))
...
class Describe90DegreeRotation(LinearTransformationScene):
    ...
    def construct(self):
        ...
        for vect, color, col in [
            (self.i_hat, X_COLOR, col1),
            (self.j_hat, Y_COLOR, col2),
        ]:
...
class TransformationsAreFunctions(Scene):
    def construct(self):
        ...
        start_arrow = Arrow(
            starting_vector.get_top(), _input.get_bottom(), color=MAROON_C
        )
        ending_arrow = Arrow(ending_vector, _output, color=BLUE)

        self.add(title)
        self.play(Write(equation), ShowCreation(func_arrow))
...
class UsedToThinkinfOfFunctionsAsGraphs(VectorScene):
    ...
    def show_inputs_and_output(self):
        ...
        arrows = VMobject(*[Arrow(LEFT, RIGHT).next_to(mob) for mob in inputs.split()])
        outputs = VMobject(
            *[
        ...
        self.wait()
        self.play(Transform(inputs.copy(), outputs), ShowCreation(arrows))
        self.wait()
...
class TryingToVisualizeFourDimensions(Scene):
    def construct(self):
        ...
        self.play(ApplyMethod(randy.look, DOWN + RIGHT), Write(formula))
        self.play(
...
class AdditivityProperty(LinearTransformationScene):
    ...
    def draw_all_vectors(self):
        ...
        sum_label = self.add_transformable_label(
            sum_vect, "%s + %s" % (v_label.expression, w_label.expression), rotate=True
        )
    ...
    def apply_transformation(self, added_anims):
        if self.nonlinear_transformation:
            ...
            self.apply_transposed_matrix(
                self.transposed_matrix, added_anims=added_anims
            )
        self.wait()
...
class MoveAroundAllVectors(LinearTransformationScene):
    ...
    def construct(self):
        ...
        if self.focus_on_one_vector:
            ...
            self.play(*[FadeOut(v) for v in vectors.split() if v is not vector])
            self.wait()
...
class ReasonForThinkingAboutArrows(LinearTransformationScene):
    ...
    def scale_and_add(self, vectors):
        ...
        self.wait()
        self.play(Transform(vectors, vectors_copy, lag_ratio=0))
        self.wait()
